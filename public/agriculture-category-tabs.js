...
function renderCategoryContent() {
  ...
  target.innerHTML = `
    ...
    <div class="universe-summary category-summary">
      ...
      <div class="universe-stat"><span>Reported area</span><strong>${area ? area.toFixed(2) : "Not available"}</strong><small>Source units may differ by dataset</small></div>
      <div class="universe-stat"><span>Reported production</span><strong>${production ? production.toFixed(2) : "Not available"}</strong><small>Only measured rows included</small></div>
    </div>
    ...
    <div class="table-wrap agriculture-table">
      ...
      <tbody>${records.map((record) => `<tr>
        ...
        <td>${record.area == null ? "Not reported" : `${categoryEscape(record.area)} ${categoryEscape(record.areaUnit)}`}</td>
        <td>${record.production == null ? "Not reported" : `${categoryEscape(record.production)} ${categoryEscape(record.productionUnit)}`}</td>
        <td>${record.productivity == null ? "Not available" : `${categoryEscape(Number(record.productivity).toFixed(2))} ${categoryEscape(record.productivityUnit)}`}</td>
