  ...
  try {
    ...
    target.innerHTML = `
      ...
      <div class="universe-summary mandi-summary">
        ...
        <div class="universe-stat"><span>Average modal</span><strong>${averageModal === null ? "Not available" : `₹${averageModal.toFixed(0)}`}</strong><small>Source market unit</small></div>
      ...
      <div class="research-panel"><h4>Recent modal-price trend</h4><p>${mandiEscape(trendNote)}</p><div class="table-wrap"><table class="mini-table"><thead><tr><th>Date</th><th>Average modal price</th></tr></thead><tbody>${trend.slice().reverse().map((item) => `<tr><td>${mandiEscape(item.date)}</td><td><strong>₹${item.value.toFixed(0)}</strong></td></tr>`).join("")}</tbody></table></div></div>
