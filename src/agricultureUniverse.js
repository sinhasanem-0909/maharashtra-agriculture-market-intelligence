...
const AGRICULTURE_DATA_SOURCES = [
  ...
  { id: "maharashtra-district-apy-2025-26", name: "Department of Agriculture, Government of Maharashtra — District-wise APY 2025–26 Third Advance Estimates", url: "https://krishi.maharashtra.gov.in/Site/Common/ViewGr.aspx?Doctype=01ce19af-d065-4817-ac1a-71c1c0f8874d%3FMenuID%3D2911", role: "District crop area, production and productivity", status: "source-identified" },
  ...
  { id: "nhb-horticulture", name: "National Horticulture Board", url: "https://nhb.gov.in/", role: "Horticulture crop area and production", status: "source-identified" }
];

const historicalByCommodity = new Map(HISTORICAL_APY.records.map((record) => [record.commodity, record]));

function buildCommodityRecord(record) {
  ...
  const productionChange = historical ? ((record.production - historical.production) / historical.production) * 100 : null;
  return {
...
function getAgricultureUniverse() {
  return STATE_APY.records.map(buildCommodityRecord);
}
